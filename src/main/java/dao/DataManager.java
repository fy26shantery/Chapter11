...
public class DataManager {
	...
	public DataManager() {
		...
		userList.add(udto);
		udto = new UserDTO("sato", "pass4", "佐藤　一男", "icon-ball", "カフェランチに行きましょう");
		userList.add(udto);
	...
	public UserDTO getLoginUser(String loginID, String userName, String password) {
		...
		for (UserDTO u : userList) {
			if (u.getLoginId().equals(loginID) && u.getUserName().equals(userName)
					&& u.getPassword().equals(password)) {
				//一致したものがあれば、そのユーザー情報の参照を戻す
				user = u;
			}
		}
		return user;
